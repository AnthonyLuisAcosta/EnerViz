...
def process_input(input_text):
    ...
    # Load ds file
    # Read the CSV file name from the command line argument
    # csv_file = sys.argv[1]
    # df = pd.read_csv('Datasets/labeled_with_neutral.csv')
    # df = pd.DataFrame(df)
    df = str(input_text)
    df = input_data = pd.DataFrame({'raw_comments': [df]})
    ...
    #ds['comment'] = ds['comment'].apply(lambda x: [correction(y) for y in x], meta=('comment', 'object'))
    ...
    #ds['comment'] = ds['comment'].apply(lambda x: [custom_stem(y) for y in x], meta=('comment', 'object'))
    ...
    #ds['comment'] = ds['comment'].apply(lambda x: [y for y in x if y not in (stpwrd)], meta=('comment', 'object'))  # Stem every word.
    # dataset['preprocessed_comments'] = dataset['preprocessed_comments'].apply(lambda x: ' '.join([word for word in x.split() if word not in (stpwrd)]))
    ...
    a = dataset['preprocessed_comments'].apply(lambda x: ' '.join([y for y in x]))
    input_text = str(a[0])
    return input_text 
